  ...
  const processingMethods = [
    {
      ...
      description: "Automatically chooses best method",
    },
    {
      value: "text",
      label: "Text Extraction",
      description: "For text-based PDFs",
    },
    {
      value: "ocr",
      label: "OCR Processing",
      description: "For image-based/scanned PDFs",
    },
    {
      value: "manual",
      label: "Manual Extraction",
      description: "Manual table creation",
    },
  ];
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (error.requiresManualExtraction) {
          toast.info(
            "This document may require manual extraction. Try switching to manual mode."
          );
        }
      }
    } catch (error) {
        ...
        <div className="mb-6 p-4 bg-gradient-to-r from-green-50 to-blue-50 border-2 border-green-200 rounded-lg">
          ...
          <div className="text-center space-y-1">
            <p className="text-green-700 font-medium">
              ✅ Bank & Financial Document Support
            </p>
            ...
            <p className="text-orange-700 font-medium">
              ✅ 50MB file limit, No authentication
            </p>
          </div>
        </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-500">
                Advanced AI-powered extraction for financial documents with OCR
                support
              </p>
              <p className="text-xs text-blue-600 mt-1">
                ✓ Auto-detect text/image PDFs ✓ Bank statements ✓ Financial
                reports ✓ Manual backup
              </p>
            </div>
          </div>
        </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    {(file.size / 1024 / 1024).toFixed(2)} MB • {documentType} •{" "}
                    {processingMethod}
                  </p>
                </div>
              </div>
        ...
        <div className="bg-white rounded-lg shadow-lg p-6">
          ...
          <div className="mb-6 p-4 bg-green-50 rounded-lg">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="font-semibold text-green-900">
                Document processed successfully - Method:{" "}
                {preview.processingMethod?.toUpperCase() || "AUTO"}
              </span>
            </div>
            <div className="text-sm text-green-800 space-y-1">
              ...
              <div>
                Extraction Quality:{" "}
                {preview.metadata?.extractionQuality || "Standard"}
              </div>
            </div>
          </div>
            ...
            <div className="mb-6 p-4 bg-yellow-50 rounded-lg">
              ...
              <div className="grid grid-cols-2 gap-4 text-sm">
                <div>
                  ...
                  {preview.previewData.financialSummary.documentCategory}
                </div>
                <div>
                  <span className="font-medium">Patterns Found:</span>{" "}
                  {preview.previewData.financialSummary.detectedPatterns}
                </div>
                <div className="col-span-2">
                  <span className="font-medium">Recommendation:</span>{" "}
                  {preview.previewData.financialSummary.recommendedAction}
                </div>
              </div>
            </div>
          )}
          ...
          {preview.previewData?.sampleContent &&
            preview.previewData.sampleContent.length > 0 && (
              <div className="mb-6">
          ...
          {/* Table Preview */}
          {preview.previewData?.tablePreview &&
            preview.previewData.tablePreview.length > 0 && (
              <div className="mb-6">
                <h4 className="font-semibold text-gray-900 mb-3">
                  📋 Detected Table Preview
                </h4>
                <div className="p-4 bg-blue-50 rounded-lg">
                  <div className="space-y-2">
                    {preview.previewData.tablePreview.map((row, index) => (
                      <div
                        key={index}
                        className="text-sm font-mono text-blue-800 border-b border-blue-200 pb-1"
                      >
        ...
        <div className="bg-white rounded-lg shadow-lg p-6">
          ...
          <div className="p-4 bg-green-50 rounded-lg">
            ...
            <div className="text-sm text-green-800">
              Rows: {extractedData.tableData.length} • Columns:{" "}
              {extractedData.tableData[0]?.length || 0}
            </div>
          </div>

          <div className="mt-4 text-center">
            <p className="text-gray-600 mb-4">
              Your manually extracted data has been saved. You can now download
              it as an Excel file.
            </p>
            <button
              onClick={() => setShowManualEditor(true)}
