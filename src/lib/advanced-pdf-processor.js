// Comprehensive polyfills for Node.js environment
if (typeof window === "undefined") {
  ...
  // Additional DOM polyfills that PDF.js might need
  ...
  // Ensure Promise.withResolvers is available globally for PDF.js
...
// Dynamic import for PDF.js to handle polyfills properly
let pdfjsLib = null;
...
async function initializePDFJS() {
  ...
  try {
    ...
    if (typeof window === "undefined") {
      // Use the worker from node_modules for server-side execution
      try {
        ...
        if (fs.existsSync(workerPath)) {
          ...
          // Fallback: disable worker for server-side processing
          pdfjsLib.GlobalWorkerOptions.workerSrc = null;
          console.warn(
            "PDF.js worker not found, running without worker (may be slower)"
          );
        }
      } catch (workerError) {
        console.warn(
          "Failed to set PDF.js worker, continuing without worker:",
          workerError.message
        );
        pdfjsLib.GlobalWorkerOptions.workerSrc = null;
      }
...
export class AdvancedPDFProcessor {
  ...
  async initializeOCR() {
    ...
    try {
      ...
      this.ocrWorker = null;
      throw new Error(
        `OCR initialization failed: ${error.message}. Please try manual extraction.`
      );
    }
  ...
  async processPDF(fileBuffer, originalFileName) {
    ...
    try {
      ...
      const textResult = await this.tryTextExtraction(
        fileBuffer,
        originalFileName
      );
      ...
      if (textResult.success && textResult.textContent.length > 100) {
        ...
        const result = await this.processTextBasedPDF(
          textResult.pdfDocument,
          originalFileName
        );
      ...
      const ocrResult = await this.processImageBasedPDF(
        fileBuffer,
        originalFileName
      );
  ...
  async processTextBasedPDF(pdfDocument, originalFileName) {
    ...
    console.log(
      `Processing ${numPages} pages from ${originalFileName} (Text-based)`
    );
    ...
    const excelBuffer = await this.createFinancialExcelFile(
      allPageData,
      originalFileName,
      documentType
    );
    ...
    const previewData = this.generateFinancialPreview(
      allPageData,
      documentType
    );
  ...
  async convertPDFToImages(fileBuffer, originalFileName) {
    ...
    try {
      ...
      console.log(
        "Attempting PDF to image conversion using pdf-to-png-converter..."
      );
      try {
        ...
        if (pageImages.length > 0) {
          console.log(
            `Successfully converted ${pageImages.length} pages using pdf-to-png-converter`
          );
          return pageImages;
        }
      ...
      console.log(
        "Attempting PDF to image conversion using PDF.js with Canvas..."
      );
      try {
        pageImages = await this.convertPDFWithCanvas(
          fileBuffer,
          tempDir,
          timestamp
        );
        if (pageImages.length > 0) {
          console.log(
            `Successfully converted ${pageImages.length} pages using PDF.js + Canvas`
          );
          return pageImages;
        }
      } catch (canvasError) {
      ...
      try {
        ...
        throw new Error(
          `Failed to write temporary PDF file: ${writeError.message}`
        );
      }
      ...
      try {
        ...
        throw new Error(
          `All PDF to image conversion methods failed. Last error: ${conversionError.message}`
        );
      }

      throw new Error(
        "Failed to convert PDF pages to images - no pages generated"
      );
    } finally {
      ...
      if (tempFilePath && fs.existsSync(tempFilePath)) {
        try {
          ...
          console.error(
            `Failed to cleanup temp PDF file: ${cleanupError.message}`
          );
        }
      }
    }
    ...
    try {
      ...
      while (retryCount < maxRetries) {
        try {
          ...
          retryCount++;
          console.warn(
            `OCR initialization attempt ${retryCount} failed:`,
            error.message
          );
          if (retryCount >= maxRetries) {
            throw new Error(
              `OCR initialization failed after ${maxRetries} attempts: ${error.message}`
            );
          }
      ...
      console.log(
        `Processing ${pageImages.length} pages from ${originalFileName} (OCR)`
      );
      ...
      for (let i = 0; i < pageImages.length; i++) {
        ...
        try {
          const pageData = await this.extractOCRPageData(
            pageImage.path,
            i + 1,
            ocrWorker
          );
          ...
          if (
            pageData.extractionMethod === "ocr-failed" &&
            pageData.error &&
            (pageData.error.includes("EOF") ||
              pageData.error.includes("worker"))
          ) {
            console.warn(
              `Worker may be corrupted, reinitializing for remaining pages...`
            );
            try {
              await this.cleanup();
              ocrWorker = await this.initializeOCR();
          ...
          if (
            pageError.message.includes("EOF") ||
            pageError.message.includes("write") ||
            pageError.code === "EOF"
          ) {
            console.warn(
              `Worker communication error on page ${
                i + 1
      ...
      return {
        ...
        metadata: {
          ...
          totalTables: allPageData.reduce(
    ...
    while (retryCount <= maxRetries) {
      try {
        ...
        const textItems = (words || [])
          .filter(
            (word) => word && word.text && word.bbox && word.confidence > 60
          )
          .map((word, index) => ({
            text: word.text.trim(),
        ...
        retryCount++;
        console.error(
          `OCR error for page ${pageNumber} (attempt ${retryCount}):`,
          error
        );
        ...
        if (
          ...
          console.warn(
            `Worker communication error detected for page ${pageNumber}, attempt ${retryCount}`
          );
  ...
  async createFinancialExcelFile(allPageData, originalFileName, documentType) {
    ...
    await this.createFinancialSummaryWorksheet(
      workbook,
      allPageData,
      originalFileName,
      documentType
    ...
    await this.createDocumentSpecificWorksheets(
      workbook,
      allPageData,
      documentType
    );
  ...
  async createFinancialSummaryWorksheet(
    workbook,
    allPageData,
    originalFileName,
    documentType
    ...
    const stats = [
      ...
      [
        "Total Text Items",
        allPageData.reduce((sum, page) => sum + page.textItems.length, 0),
      ],
      [
        "Total Tables Detected",
        allPageData.reduce((sum, page) => sum + (page.tables?.length || 0), 0),
      ],
      [
        "Processing Method",
        allPageData[0]?.extractionMethod || "text-extraction",
      ],
  ...
  async createDocumentSpecificWorksheets(workbook, allPageData, documentType) {
    ...
    if (config) {
      ...
      await this.populateDocumentSpecificData(
        worksheet,
        allPageData,
        documentType
      );
  ...
  async createManualExtractionTemplate(workbook, documentType) {
    ...
    instructionsCell.value =
      "Use this template to manually extract data when automatic processing fails";
    ...
    const config =
      this.financialPatterns[documentType] || this.financialPatterns.general;
  ...
  generateFinancialPreview(allPageData, documentType) {
    ...
    allPageData.forEach((pageData, index) => {
      ...
      // Table preview
      if (
        preview.tablePreview.length === 0 &&
        pageData.tables &&
        pageData.tables.length > 0
      ) {
        const firstTable = pageData.tables[0];
        preview.tablePreview = firstTable.rows
          .slice(0, 3)
          .map((row) => row.map((item) => item.text || item).join(" | "));
      }
    });
    ...
    preview.financialSummary = {
      ...
      detectedPatterns:
        this.financialPatterns[documentType]?.patterns.length || 0,
  ...
  async extractCompletePageData(page, pageNum) {
    ...
    const pdfjsLib = await initializePDFJS();
    const images = this.detectImages(operatorList, pdfjsLib);
    const processedTextItems = this.processTextItems(
      textContent.items,
      viewport
    );
  ...
  detectImages(operatorList, pdfjsLib) {
    const images = [];
    ...
    for (let i = 0; i < ops.length; i++) {
      if (
        ops[i] === pdfjsLib.OPS.paintImageXObject ||
  ...
  detectColumns(tableRows) {
    ...
    return uniquePositions.map((x, index) => ({
      ...
      x,
      width:
        index < uniquePositions.length - 1
          ? uniquePositions[index + 1] - x
          : 100,
    }));
  }
