  ...
  async processPDF(fileBuffer, originalFileName) {
    ...
    try {
      ...
      for (let pageNum = 1; pageNum <= numPages; pageNum++) {
        ...
        if (this.debugMode) {
          console.log(
            `Page ${pageNum}: ${pageData.textItems.length} text items, ${pageData.images.length} images, ${pageData.tables.length} tables`
          );
        }
      }

      const excelBuffer = await this.createComprehensiveExcelFile(
        allPageData,
        originalFileName
      );
  ...
  async createSummaryWorksheet(workbook, allPageData, originalFileName) {
    ...
    const stats = [
      ...
      [
        ...
        allPageData.reduce((sum, page) => sum + page.tables.length, 0),
      ],
      [
        "Total Images Detected",
        allPageData.reduce((sum, page) => sum + page.images.length, 0),
      ],
      [
        "Total Form Fields",
        allPageData.reduce((sum, page) => sum + page.formFields.length, 0),
      ],
    ...
    allPageData.forEach((pageData) => {
      ...
      if (pageData.tables.length > 0) {
        ...
        pageData.tables.forEach((table, tableIndex) => {
          // Table identifier
          worksheet.getCell(`A${currentRow}`).value = `Table ${
            tableIndex + 1
          }:`;
  ...
  async createPageWorksheets(workbook, allPageData) {
    allPageData.forEach((pageData) => {
      ...
      const stats = [
        ...
        [
          "Page Dimensions",
          `${Math.round(pageData.viewport.width)} x ${Math.round(
